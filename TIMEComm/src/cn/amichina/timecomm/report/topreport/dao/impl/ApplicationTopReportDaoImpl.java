...
@Repository
public class ApplicationTopReportDaoImpl implements TopReportDao {
	@Override
	public QueryResult<TopReport> pageQueryTopReport(int startIndex,
			int pageSize, Date startDate, Date endDate) {
			QueryRunner runner =new QueryRunner();
			String sql ="select B.name as top,sum(innum) as download, sum(outnum) as upload,sum(innum+outnum) as total from pmreport2.vcdaytj_sp as A,pmconf.sysservices as B where B.id=A.serrvice and sj >= ? and sj <= ? group by B.name order by download desc limit ? offset ?";
			List<TopReport> list;
			try {
			...
			String sql_TotalRecord="select count(serrvice) from pmreport2.vcdaytj_sp where sj >= ? and sj <= ? and servicename is not null ";
